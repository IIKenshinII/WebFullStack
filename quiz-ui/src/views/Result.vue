  ...
  <ul>
    <li v-for="question,index in questions" :key="question.position" :value="question.position" style="border-style: solid;border-color:black;margin-bottom:2%;" >
        ...
        <div v-for="answer,i in question.possibleAnswers" :key="answer.id">
          <p v-if="questionGoodAnswers[index].wasCorrect==true && i+1==questionGoodAnswers[index].correctAnswerPosition" class="good" > {{ answer.text }}<i class="bi bi-check" style="font-size:2em;color:green;"></i></p>
          <p v-else-if="i+1==answers[index]" class="bad" > {{ answer.text }} <i class="bi bi-x" style="font-size:2em;color:red;"></i></p>
